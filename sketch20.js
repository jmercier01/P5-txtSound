//hypotrochoids  (Parametric functions) voir
...
//   myFont = loadFont('ASSET/FONTS/AbrilFatface-Regular.ttf');
  xml = loadXML('ASSET/COLORS/antartica.xml');
...
function render(percent) {
    ...
    for (let a = 0; a < TWO_PI; a += pasBo) {
        ...
        box(x*pow(percent,0.1)*0.8,6*valSize,10);
        push();
        rotateX(a);
        translate(0,50,0);
        box(x*pow(percent,0.8)*0.3,3*valSize,5);
        rotateY(a);
        translate(0,80,100);
        box(x*pow(percent,0.38)*0.2,3*valSize,5);
        translate(0,50,0);
        box(x*pow(percent,0.8)*0.3,3*valSize,5);
        rotateY(a);
        translate(0,80,50);
