...
let endPhi;
let diffPhi;
var levelF,pasBo, freqX, freqY, freqZ, scaleZ, modFreqX,modFreqY,modFreqZ, noiseMax, totalFrames, spd, rotX, rotY, rotZ, size, rotFxX, rotFixY,rotFixZ, valShearX,valShearY;
let canvas;
let angleGen = 10;
let pan = 0.1;
var gui, gui2;
...
//   myFont = loadFont('ASSET/FONTS/AbrilFatface-Regular.ttf');
  // xml = loadXML('ASSET/COLORS/chine.xml');
  xml = loadXML('ASSET/COLORS/polar.xml');
  // sound = loadSound('../_MP3/Absynthe-Minded-MyHeroicsPartOne.mp3');
  // sound = loadSound('../_MP3/DaftPunk-OneMoreTime.mp3');
//   sound = loadSound('ASSET/MP3/the-jimi-hendrix-experience_-_foxey-lady.mp3');
//   sound = loadSound('../_MP3/TheKills-PullAU.mp3');
...
function setup() {
    ...
    changeVals();
    noiseMax = 0;
    totalFrames = 400;//round(random(800,1600));
    scaleZ = .5;
    pasBo = 3.6; //random(0.4,0.6);
    posLight = 1;
    spd = 0.009;
   ...
   size = 1;
    ...
    canvas = createCanvas(827, 1170, WEBGL).canvas;
...
function draw() {
    ...
    level = amplitude.getLevel()*levelF;
...
function render(percent) {
    ...
    for (let a = 0; a < TWO_PI; a += pasBo) {
        ...
        let levMp= map(level,0,4,0,100);
        stroke(255,255,255,levMp);
        
        beginShape();
        vertex(0, 0, 0);
        bezierVertex(100, -x*level, 0, 100, x*level, 0, 200,0,0);
        endShape();
        
        push();
        translate(50*level,0,0);
        rotateX(angle);
        box(5);
        pop();
