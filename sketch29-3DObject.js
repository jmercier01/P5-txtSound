...
//   myFont = loadFont('ASSET/FONTS/AbrilFatface-Regular.ttf');
  xml = loadXML('ASSET/COLORS/BW.xml');
...
function setup() {
    ...
    pg.background(colorier(round(random(4))));
...
function render(percent) {
    ...
    for (let a = 0; a < TWO_PI; a += pasBo) {
        ...
        switch(true){
          case (level<.5):
              box(20,150,200*pow(percent,10));
            // sphere(100*pow(percent,0.1));
            break;
          case (level>2 && level<2.5):
            box(200*pow(percent,10),150,20);
            break;
          default:
            //   ellipsoid(x*pow(percent,0.1),50+4*pow(valSize,0.8),2+10*pow(valSize,0.8));
            // box(100,200*pow(percent,10),20);
          break;
        }
