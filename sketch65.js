...
//   xml = loadXML('ASSET/COLORS/noirblanc-Duo3.xml');
  xml = loadXML('ASSET/COLORS/peace.xml');
  sound = loadSound('../_MP3/the-kills-pull-a-u.mp3');
}
...
function render(percent) {
    ...
    let b2 = ease(linearstep(percent,.66,.99),8);
    let b3 = ease(linearstep(percent,0.1,0.33),8);
    
    // //Timeline3
    let c = ease(linearstep(percent,0,1),12);
    
  
    // translate(b2*posx/2,b2*posy/2,0);
    // let pos = map(sin(angle),-1,1,-300,300);
    // let pos = map(cos(angle),-1,1,-300,300);
    // translate(pos,pos,0);
    // rotateX(counter*.005);
    // rotateY(b2);
    // rotateX(b3*2);
    ...
    for (let a = 0; a < TWO_PI; a += TAU/divis) {
        ...
        if(level>8){changeVals();}
        
        box(valSize*10+b2,1,x/30);
        // box(100,100,100);
