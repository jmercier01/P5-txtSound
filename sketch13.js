...
function preload() {
  typo = loadFont('ASSET/FONTS/Oswald-Regular.ttf');
  img = loadImage('ASSET/IMG/Banana-Single.jpg');
  xml = loadXML('ASSET/COLORS/banana.xml');
...
function draw() {
    ...
    if(seeImg == 1){
      ...
      textAlign(CENTER);
      fill(20);
      text('B A N A N A',0,0);
    } else {
      fill(colorier(couleurFd));
      rect(-(img.width*0.3)/2, -(img.height*0.3)/2 , img.width*0.3, img.height*0.3);
    }
...
function render(percent) {
    ...
    for (let a = 0; a < TWO_PI; a += pasBo) {
        ...
        ellipsoid(x*pow(percent,.9)*0.7, 4*pow(percent,.9), 4*pow(percent,.1));
        push();
        rotateY(pow(a,0.1));
        ellipsoid(5, 3*pow(percent,.1), 3*pow(percent,.9));
        pop();
