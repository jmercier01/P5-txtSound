...
function preload() {
  ...
  img = loadImage('ASSET/IMG/photo-1579084659629-d24f785cfec5.jpeg')
}
...
function setup() {
    ...
    changeVals();
    noiseMax = 0.6;
    totalFrames = round(random(500,1000));
    scaleZ = 1;
...
function render(percent) {
    ...
    for (let a = 0; a < TWO_PI; a += pasBo) {
        ...
        translate(0,liss.x*valSize,liss.y*valSize);
        ...
        if (mouseIsPressed) {
            ...
            ellipsoid((liss.x*percent*valSize*0.03)*szuoff,(liss.x*percent*valSize*0.03)*szuoff,(liss.x*percent*valSize*0.03)*szuoff);
            // box((liss.x*percent*valSize*0.005)*szuoff,(liss.y*percent*valSize*0.05)*szuoff);
        }
