...
function preload() {
  xml = loadXML('ASSET/COLORS/hasard01.xml');
...
function render(percent) {
    ...
    for (let a = 0; a < TWO_PI; a += pasBo) {
        ...
        push();
        beginShape();
        translate(sin(frameCount*.1)*100,cos(frameCount*.1)*100,0);
        stroke(colorier(0));
        vertex(0, 0, 0);
        bezierVertex(100, -x*level, 0, 100, x*level, 0, 200,0,0);
        vertex(0, 0, 0);
