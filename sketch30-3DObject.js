...
//   myFont = loadFont('ASSET/FONTS/AbrilFatface-Regular.ttf');
  xml = loadXML('ASSET/COLORS/Should_Have_Known_Better.xml');
  sound = loadSound('ASSET/MP3/SY-Skink.mp3');
  raptor = loadModel("ASSET/OBJ/male_head.obj");
  img = loadImage("ASSET/IMG/TXT-SOUND-TXT03-04.jpg");
}
...
function setup() {
    ...
    changeVals();
    noiseMax = 0.7;
    totalFrames = round(random(800,1600));
    scaleZ = 0.2;
    ...
    background(colorier(4));
    noise = new OpenSimplexNoise();
    
    makeGui();
    
    pg = createGraphics(400,400,WEBGL);
    pg.image(img,0,0,40,40);
...
function render(percent) {
    ...
    if(angle != 0){
      ...
      rotateZ(rotFixZ+rotZ*2+cr0);
    }

    
    let rad = map(sin(angle),-1,1,-width,height);
    ...
    for (let a = 0; a < TWO_PI; a += pasBo) {
        ...
        translate(x,y*pow(valSize,0.7)/2,z*valSize/2);
        
        
        switch(true){
          case (level<2):
            box(0.4*(pow(a,4)),10,10);
            break;
          case (level>2 && level<2.5):
            box(0.4*(pow(a,4)),10,100);
            break;
          default:
            //   ellipsoid(x*pow(percent,0.1),50+4*pow(valSize,0.8),2+10*pow(valSize,0.8));
          break;
        }
        // ellipsoid(x*pow(percent,0.1),pow(valSize,0.8),20);
        pop();
