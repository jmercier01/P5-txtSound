...
function preload() {
  ...
  xml = loadXML('ASSET/COLORS/bistrot.xml');
...
function render(percent) {
    ...
    push();
    let easing = ease(linearstep(percent,.25,.45),8);
    let easing2 = ease(linearstep(percent,.3,.7),8);
    
    let min = -height/2+300;
    let max = height/2-300;
    let up = map(percent,0,1,min,max);
    if(up>max-1) changeVals();
    let prog = map(percent,0,1,10,5);
    let prog2 = map(percent,0,1,5,0);
    
    translate(sin(angle*prog)*easing*10,up,0);
    // rotateY(angle*prog2);
    // rotateX(angle);
