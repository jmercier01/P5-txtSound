...
let endPhi;
let diffPhi;
var pasBo, freqX, freqY, freqZ, scaleZ, modFreqX,modFreqY,modFreqZ, noiseMax, totalFrames, spd, rotX, rotY, rotZ, size, rotFixX, rotFixY,rotFixZ, valShearX,valShearY;
let canvas;
let angleGen = 1;
let pan = 0.1;
var gui, gui2;
...
function preload() {
  xml = loadXML('ASSET/COLORS/candy.xml');
  sound = loadSound('ASSET/MP3/PF-GreatGigITHSky.mp3');
}
...
function setup() {
    ...
    velocity = createVector(0.0,-0.1);
    
    background(colorier(3));
...
function render(percent) {
    ...
    for (let a = 0; a < TWO_PI; a += pasBo) {
        ...
        if(a<TWO_PI/2){
          ...
          valRad = map(a,0,TWO_PI,0,1);
        } else{
          valCol = map(a,0,TWO_PI,255,180);
          valRad = map(a,0,TWO_PI,1,0);
        }
        ...
        let inv = (1/pow(frameCount,4.0));
        let mapSize = map(level,0,3.4,0,200);
        // rotFixX = map(level,0,3.4,0,0.5);
        // if(level>0.5){
        //     inv *= -2;
        //     // translate(x,0,z);
        ...
        // ellipsoid(pow(valRad*30,0.99),pow(valRad*30,0.99),pow(valRad*30,0.99));
        ellipsoid(10, 10, y*valSize);
...
function changeVals(){
    ...
    freqZ = round(random(10));
    modFreqX =  round(random(20));
