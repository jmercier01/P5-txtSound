...
//   myFont = loadFont('ASSET/FONTS/AbrilFatface-Regular.ttf');
  // xml = loadXML('ASSET/COLORS/entropy.xml');
  xml = loadXML('ASSET/COLORS/pink.xml');
...
function setup() {
    ...
    // movex = random(-10,10), movey = random(-10,10);
    freqX = random(10);
    freqY = random(10);
    freqZ = random(10);
    modFreqX =  random(10);
    ...
    changeVals();
    noiseMax = 0.7;
    totalFrames = 800;//round(random(800,1600));
    scaleZ = .2;
    pasBo = 0.05;//random(0.4,0.6);
    posLight = 1;
    spd = 0.009;
    rotX = 0.02;
    rotY = 0.7;
...
function render(percent) {
    ...
    for (let a = 0; a < TWO_PI; a += TAU/10) {
        ...
        if(level<1){
            //last valeur
            //translate(0,y*6,0);
            translate(0,x*pow(valSize,0.7)/4,y*valSize/4);
        } 
        // else
        // {
        //   if(level<0.5){
        ...
        rotateX(a);
        translate(0,50,0);
        // ellipsoid(x*pow(percent,0.8)*2,2*valSize,20);
        box(x*pow((valSize),3)*.01,2,2);
