...
//   sound = loadSound('ASSET/MP3/01_-_the_transisters_-_under_control.mp3');
}
...
let capturer = new CCapture( { 
  ...
  function setup() {
    ...
    //   movex = random(-2,2), movey = random(-2,2);
      ...
      // size = random(10,20);
      size = 1;
      canvas = createCanvas(800, 800, WEBGL).canvas;
      // ortho(-width / 2, width / 2, height / 2, -height / 2, 0, 500);
    //   setAttributes('antialias', true);
      // background(colorier(round(random(4))));
      background(colorier(2));
  ...
  function render(percent) {
      ...
      for (let a = 0; a < TWO_PI; a += pasBo) {
          ...
          box(x*pow(percent,0.3)*0.4,6*valSize,10);
        //   push();
        //   rotateX(a);
        //   translate(0,120,0);
        //   box(x*pow(percent,0.8)*0.3,3*valSize,10);
        //   pop();
          pop();
