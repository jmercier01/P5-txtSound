...
//   myFont = loadFont('ASSET/FONTS/AbrilFatface-Regular.ttf');
  xml = loadXML('ASSET/COLORS/freshartwork2.xml');
...
function setup() {
  ...
  movex = random(-1,1), movey = random(-1,1);
    // freqX = random(10);
    // freqY = random(10);
    // freqZ = random(10);
    // modFreqX =  random(10);
    ...
    valShearX = 0;
    valShearY = 0;
    posx=random(-limite,limite);
    posy=random(-limite,limite);
...
function render(percent) {
    ...
    for (let a = 0; a < TWO_PI; a += pasBo) {
        ...
        box(x*pow(percent,0.1)*0.5,valSize,10);
