...
let endPhi;
let diffPhi;
var pasBo, volume = 0, freqX, freqY, freqZ, scaleZ, modFreqX,modFreqY,modFreqZ, noiseMax, clearinglfralFrames, spd, rotX, rotY, rotZ, size, rotFxX, rotFixY,rotFixZ, valShearX,valShearY;
let canvas;
let angleGen = 1;
let pan = 0.1;
var gui, gui2;
...
function preload() {
  ...
  xml = loadXML('ASSET/COLORS/volca.xml');
  sound = loadSound('../_MP3/FujiyaMiyagiAnkleInjuries.mp3');
...
function render(percent) {
    ...
    push();
    let easing = ease(linearstep(percent,0,.75),4);
    let easing2 = ease(linearstep(percent,.3,.7),8);
    
    let min = -height/2+300;
    let max = height/2-350;
    let prog = map(percent,0,1,10,100);
    let spix = cos(angle*3)*prog;
    let spiy = sin(angle*2)*max;
    // let up = map(percent,0,1,min,max);
    // if(spiy>max-1) changeVals();
    // let prog2 = map(percent,0,1,5,0);
    
    translate(spix,spiy,0);
    // rotateY(angle*prog2);
    // rotateX(angle);
    ...
    if(angle != 0){
        // rotateY(TAU/(PI));
        rotateY(angle*4+rotY);
        // rotateZ(angle+rotZ);
        rotateZ(angle);
    }

    algo(percent, angle, uoff, voff, rad);

  pop();
}
function algo(percent,angle,uoff,voff,rad){
    for (let a = 0; a < TWO_PI; a += TAU/12) {
        let xoff = map(cos(a), -1, 1, 0, noiseMax);
        let yoff = map(sin(a), -1, 1, 0, noiseMax);
        
        let cr = map(noise.noise4D(xoff, yoff, uoff, voff), -1, 1,-rad*5,rad*3);
