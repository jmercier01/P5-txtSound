...
function preload() {
  ...
  xml = loadXML('ASSET/COLORS/sombre.xml');
  sound = loadSound('../_MP3/queens-of-the-stone-age-im-designer-hq.mp3');
...
function render(percent) {
    ...
    volume = sin(angle*10)*15;
    level = amplitude.getLevel()*volume;
    
    let rad = map(sin(angle),-1,1,0,width/7);
    ...
    translate(map(percent,0,1,-200,200),map(percent,0,1,-300,400),0);
    // rotateY(angle*prog2);
    // rotateX(angle);
